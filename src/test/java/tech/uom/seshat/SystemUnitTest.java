...
public final strictfp class SystemUnitTest {
            ...
            if (value instanceof SystemUnit<?>) {
                ...
                if (related != null) {
                    ...
                    for (final ConventionalUnit<?> r : related) {
                        ...
                        assertTrue(symbol, r.toTarget instanceof LinearConverter);
                    }
                }
            }
        }
    ...
    public void testMultiply() {
        ...
        assertEquals(expected, Units.METRE.multiply(Units.SECOND).getBaseUnits());
    }
    ...
    public void testGetBaseDimensions() {
        ...
        assertEquals(Collections.singletonMap(Units.METRE, 3), Units.CUBIC_METRE.getBaseUnits());

        final Map<Unit<?>,Integer> expected = new HashMap<>(4);
        assertNull(expected.put(Units.KILOGRAM, 1));
        assertNull(expected.put(Units.METRE,    1));
        assertNull(expected.put(Units.SECOND,  -2));
        assertEquals(expected, Units.NEWTON.getBaseUnits());
    }
    ...
    /**
     * Tests {@link SystemUnit#asType(Class)} for a quantity unknown to Seshat.
     */
    @Test
    public void testAsTypeForNewQuantity() {
        /*
         * Tests with a new quantity type unknown to Seshat.
         * Seshat cannot prove that the type is wrong, so it should accept it.
         */
    ...
    /**
     * A dummy quantity type for tests using a quantity type unknown to Seshat.
     */
