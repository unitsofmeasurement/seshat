...
class SexagesimalConverter extends AbstractConverter {
     ...
     * {@code "D.MMm"} {@code AngleFormat} pattern.
     *
     ...
     *
     * Using this unit is loosely equivalent to formatting decimal degrees with the
     * {@code "D.MMSSs"} {@code AngleFormat} pattern.
     *
     * <p>This unit is non-linear and not practical for computation. Consequently, it should be
     ...
     *
     * Using this unit is loosely equivalent to formatting decimal degrees with the
     * {@code "DMMSS.s"} {@code AngleFormat} pattern.
     *
     * <p>This unit is non-linear and not practical for computation. Consequently, it should be
     * avoided as much as possible. This pseudo-unit is defined only because extensively used in
    ...
    private static final class Inverse extends SexagesimalConverter {
        ...
        private static double fixRoundingError(double remainder, final double magnitude) {
            ...
            if (p < 0 && p >= -MathFunctions.SIGNIFICAND_SIZE) {    // Precision is a fraction digit >= Math.ulp(1).
                p = MathFunctions.toExp10(-p);                      // Positive power of 10, rounded to lower value.
         ...
         */
        private static IllegalArgumentException illegalField(final double value) {
            return new IllegalArgumentException(Errors.format(Errors.Keys.IllegalArgumentValue_2, "angle", value));
        }
    }
    ...
    private static double truncate(final double value) {
        return (Double.doubleToRawLongBits(value) & Long.MIN_VALUE) == 0 ? Math.floor(value) : Math.ceil(value);
    }
}
